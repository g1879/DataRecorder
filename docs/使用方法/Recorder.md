...
?>**Tips：**<br>接收`dict`数据时，会记录其`value`值。如果一个新文件，接收的第一条数据是`dict`格式，会自动根据其`key`值创建表头。
...
`Recorder`支持 xlsx、csv、json、txt 四种记录格式。程序会根据文件后缀判断使用哪一种格式。如果是其它后缀的文件，会以 txt 方式进行记录。
